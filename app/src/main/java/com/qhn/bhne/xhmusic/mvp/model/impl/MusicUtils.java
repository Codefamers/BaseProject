...
import com.qhn.bhne.xhmusic.utils.SortOrder;
...
public class MusicUtils {
    ...
    public static List<SearchAlbum> queryAlbums(Context context) {
        ...
        List<SearchAlbum> list = getAlbumList(cr.query(Albums.EXTERNAL_CONTENT_URI, proj_album,
                where.toString(), null, SortOrder.AlbumSortOrder.ALBUM_A_Z));
        return list;
    ...
    private static ArrayList<SongInfo> queryMusic(Context context, String id, int from) {
        ...
        final String songSortOrder = SortOrder.AlbumSortOrder.ALBUM_A_Z;


        switch (from) {
            case START_FROM_LOCAL:
                ArrayList<SongInfo> list3 = getMusicListCursor(cr.query(uri, proj_music,
            ...
            case START_FROM_FOLDER:
                ArrayList<SongInfo> list1 = new ArrayList<>();
                ArrayList<SongInfo> list = getMusicListCursor(cr.query(Media.EXTERNAL_CONTENT_URI, proj_music,
                        select.toString(), null,
                        null));
                for (SongInfo songInfo : list) {
                    if (songInfo.getFolderPath().substring(0, songInfo.getFolderPath().lastIndexOf(File.separator)).equals(id)) {
                        list1.add(songInfo);
                    }
                }
                return list1;
    ...
    public static ArrayList<SongInfo> getMusicLists(Context context, long[] id) {
        ...
        while (cursor.moveToNext()) {
            SongInfo songInfo = cursorToSongInfo(cursor);

            for (int i = 0; i < id.length; i++) {
                if (id[i] == songInfo.getSongID()) {
                    musicList.set(i, songInfo);
                }
            }
        }
        cursor.close();
    ...
    private static ArrayList<SongInfo> getMusicListCursor(Cursor cursor) {
        ...
        ArrayList<SongInfo> musicList = new ArrayList<>();
        while (cursor.moveToNext()) {
            musicList.add(cursorToSongInfo(cursor));
        }
        cursor.close();
        return musicList;
    }
    ...
    private static SongInfo cursorToSongInfo(Cursor cursor) {
        ...
        songInfo.setTimeLength(cursor.getInt(cursor
                .getColumnIndex(Media.DURATION)));

        songInfo.setSongName(cursor.getString(cursor
                .getColumnIndex(Media.TITLE)));
        songInfo.setSingerName(cursor.getString(cursor
                .getColumnIndex(Media.ARTIST)));
        songInfo.setSingerId((int) cursor.getLong(cursor.getColumnIndex(Media.ARTIST_ID)));

        String filePath = cursor.getString(cursor
                .getColumnIndex(Media.DATA));
        songInfo.setFolder(filePath.substring(0, filePath.lastIndexOf(File.separator)));
        songInfo.setFolderPath(filePath);
        songInfo.setFileSize(cursor.getInt(cursor
                .getColumnIndex(Media.SIZE)));
        songInfo.setLocal(true);
        return songInfo;
    }
    ...
    public static List<SearchAlbum> getAlbumList(Cursor cursor) {
        ...
        while (cursor.moveToNext()) {
            ...
            info.setAlbumid(cursor.getInt(cursor.getColumnIndex(Albums._ID)));
            info.setSongcount(cursor.getInt(cursor
                    .getColumnIndex(Albums.NUMBER_OF_SONGS)));
            info.setImgurl(getAlbumArtUri(info.getAlbumid()) + "");
            info.setSingername(cursor.getString(cursor.getColumnIndex(Albums.ARTIST)));
            list.add(info);
        }
        cursor.close();
        return list;
