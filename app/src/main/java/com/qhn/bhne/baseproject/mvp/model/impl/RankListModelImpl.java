 ...
 * on 2017/3/3 0003.
 */
public class RankListModelImpl implements MusicListModel<List<Songs>>,Parcelable {
    @Inject
    public RankListModelImpl() {
    }
    ...
    public Subscription loadMusicList(Map<String, Integer> paramsMap, final boolean isShowProgress, final RequestCallBack<List<Songs>> listener) {
        return RetrofitManager.getInstance(HostType.MOBILECDN_KUGOU)
                .getMusicRankDetailsObservable(paramsMap)
    ...
    protected RankListModelImpl(Parcel in) {
    }

    public static final Parcelable.Creator<RankListModelImpl> CREATOR = new Parcelable.Creator<RankListModelImpl>() {
        @Override
        public RankListModelImpl createFromParcel(Parcel source) {
            return new RankListModelImpl(source);
        }

        @Override
        public RankListModelImpl[] newArray(int size) {
            return new RankListModelImpl[size];
        }
    };
}
